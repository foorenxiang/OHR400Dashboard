...
for numHiddenLayers in range(1, 1 + maxHiddenLayers):
	...
	# use gaussian RBF
	srhl_rbf = RBFRandomHiddenLayer(n_hidden=numHiddenLayers*2, gamma=0.1, random_state=0)

	log_reg = LogisticRegression(solver='liblinear')
	#ELMRegressor(tanh, regressor = log_reg) and ELMRegressor(srhl_sinsq) transfer functions are not compatible
	#########from plot_elm_comparison.p#########

	regressors = [ELMRegressor(),ELMRegressor(srhl_tanh), ELMRegressor(srhl_tribas),ELMRegressor(srhl_hardlim),ELMRegressor(srhl_rbf)]
	...
	print()
	allTrainedELMModels[numHiddenLayers] = TrainedELMModels #save models from current iteration of hidden layers
#########train all elm models#########
...
savedGPSSpeedModel = dump(modelSave, 'elmGPSSpeedModel.model')
...
if not kdbSource:
	...
	for kernelName, colour in zip(hiddenLayerAndKernelRMSEs.keys(), colours):
		...
		if usePCA:
			...
			plotTitle += " no PCA"
		plotTitle += " Lowest RMSE: " + strFloat(min(hiddenLayerAndKernelRMSEs[kernelName]))
		plt.title(plotTitle)
		plt.xlabel('Hidden layers')
		plt.ylabel('RMSE')
		ax.scatter(list(range(1,maxHiddenLayers+1)), hiddenLayerAndKernelRMSEs[kernelName], s=10, c=colour, marker="s", label=kernelName)
...
if not kdbSource:
	...
	for sample in comparisons:
		print(strFloat(sample[0]) + "kph || " + strFloat(sample[1]) + "kph || " + strFloat(sample[2]) + "kph")
