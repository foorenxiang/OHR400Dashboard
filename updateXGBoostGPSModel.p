...
for booster in boosters:
    for treeDepth in range(4, 10):
        ...
        for n_estimator in np.logspace(start=math.log10(2), stop=math.log10(350), num=10):
            ...
            model = XGBRegressor(booster=booster, max_depth=treeDepth,
                                 objective='reg:squarederror', n_estimators=int(n_estimator), verbosity=1)

            model.fit(trainX, np.ravel(trainy))
            models.append(model)
            modelNames.append(boosterVariation)

            savedGPSSpeedModelSVR = dump(
                model, './models/XGBoostGPSSpeedModel.model')
