...
/must be float to allow conversion from table to matrix
update timeDeltaus:`float$timeus[i+1]-timeus[i] from `visualisationTable; 
/////removes samples with missing features in dataset/////
/ delete rows where there are no rcCommands0
delete from `visualisationTable where rcCommand0 = 0n ;
/delete rows where there are no timeDeltaus 
delete from `visualisationTable where timeDeltaus = 0n;
/delete rows where there are skips in time delta due to log transition
delete from `visualisationTable where timeDeltaus <0; 
