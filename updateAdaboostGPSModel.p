...
baseEstimators = [None, KNeighborsRegressor(3), SVR(kernel="linear", C=0.025), SVR(gamma=2, C=1)]#, GaussianProcessRegressor(1.0 * RBF(1.0)), DecisionTreeRegressor(max_depth=5), RandomForestRegressor(max_depth=5, n_estimators=10, max_features=1), MLPRegressor(alpha=1, max_iter=1000)]


baseEstimatorsNames = ['DecisionTreeRegressor(3)', 'KNeighborsRegressor(3)', 'SVR(kernel="linear", C=0.025)', 'SVR(gamma=2, C=1)']#, 'GaussianProcessRegressor(1.0 * RBF(1.0))', 'DecisionTreeRegressor(max_depth=5)', 'RandomForestRegressor(max_depth=5, n_estimators=10, max_features=1)', 'MLPRegressor(alpha=1, max_iter=1000)']
...
for baseEstimator, baseEstimatorsName in zip(baseEstimators, baseEstimatorsNames):
	for treeDepth in range(4,10):
		...
		for n_estimator in np.logspace(start = math.log10(10), stop = math.log10(500), num=10):
			...
			model = AdaBoostRegressor(base_estimator = baseEstimator, n_estimators = n_estimatorInt)
