...
fileName = 'updateRegressionLSTM.p'
...
trainX = trainingDataTrain.drop(trainingDataTrain.columns[-1], axis=1, inplace = False)
trainy= trainingDataTrain.iloc[:,-1].to_frame() # get expected throttle column
testX = trainingDataTest.drop(trainingDataTest.columns[-1], axis=1, inplace = False)
...
model.fit(trainX, trainy, epochs=trainingEpochs, batch_size=batchSize, verbose=2)
