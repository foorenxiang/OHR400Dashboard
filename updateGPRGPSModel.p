...
# gp.kernels.ExpSineSquared(), gp.kernels.DotProduct()
kernels = [gp.kernels.RationalQuadratic()]
...
for kernel, kernelName in zip(kernels+convolutedKernels, kernelNames + convolutedKernelNames):
    ...
    model = gp.GaussianProcessRegressor(
        kernel=kernel, alpha=0.001, normalize_y=True)
...
if kdbSource == False:
    ...
    plt.ylabel('RMSE')
    x = range(0, len(RMSEs))
    plt.xticks(ticks=x, labels=kernelNames +
               convolutedKernelNames, rotation=20)
