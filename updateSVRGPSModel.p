...
for kernel in kernels:
    for degree in range(3, 11):
        print("SVR Kernel used:")
        kernelVariation = kernel
        if kernel == 'poly':
            kernelVariation += str(degree)
        print(kernelVariation)
        model = SVR(kernel=kernel, gamma='scale', degree=degree)
